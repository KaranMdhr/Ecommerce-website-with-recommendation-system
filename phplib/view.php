...
{
	global $con;
	$sql = "SELECT * FROM `mobee_product` ORDER BY RAND() DESC LIMIT 20";
	$result = $con->query($sql);
...
function displayRecommendation($product_id)
{
	$url = 'http://localhost/shoppers/algorithms.php?id=' . $product_id;
					...
					<div class='agile_top_brand_left_grid1'>
						<figure>
							<div class='snipcart-item block'>
								<div class='snipcart-thumb'>
									<?php echo "
								<a href='singleproduct.php?id=" . base64_encode($product['product_id']) . "&name=" . base64_encode($product['product_name']) . "'>";

									?>

									<img src='admin/Upload/Products/<?php echo htmlspecialchars($product['image']); ?>'
										style='height:195.5px;width:124.75;' alt='' class='img-responsive' />
									</a>
									<p><?php echo htmlspecialchars($product['product_name']); ?></p>
									<h4>Rs. <?php echo htmlspecialchars($product['selling']); ?>/-
										<span>Rs.<?php echo htmlspecialchars($product['price']); ?>/-</span>
									</h4>
									<strong>Product Code :</strong><?php echo htmlspecialchars($product['SKU']); ?>
							</div>
							<div class='snipcart-details top_brand_home_details'>
								<?php
								echo "
											<a href='singleproduct.php?id=" . base64_encode($product['product_id']) . "&name=" . base64_encode($product['product_name']) . "' class='snipcart-details'>Add To Cart</a>
									";
								?>
							</div>
					</div>
...
//recomendation
function displayprice($product_id)
{
    // URL to fetch recommendations based on price range
    $url = 'http://localhost/shoppers/price_recommendations.php?id=' . $product_id;
    $response = file_get_contents($url);
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_product` WHERE `product_id`='" . $id . "'  ";
	$result = $con->query($sql);
	if ($result->num_rows > 0) {
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_product` ORDER BY RAND() LIMIT $limit";
	$result = $con->query($sql);
...
function viewproductbycategory($category_id)
{
	...
	if ($result->num_rows > 0) {
		while ($row = $result->fetch_assoc()) {
			$sql1 = "SELECT * FROM mobee_product WHERE product_id='" . $row['product_id'] . "' ";
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_product` ORDER BY RAND() LIMIT 20";
	$result = $con->query($sql);
...
function viewcartitem()
{
	if (isset($_SESSION['uniqueSessionID'])) {
		global $con;
		$total = 0;
		$sql = "SELECT * FROM `mobee_cart` WHERE `User_ID`='" . mysqli_real_escape_string($con, $_SESSION['uniqueSessionID']) . "' ";
		$result = $con->query($sql);
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				$sqlcommand = "SELECT * FROM `mobee_product` WHERE `product_id`='" . mysqli_real_escape_string($con, $row['product_id']) . "' ";
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_user` WHERE `customer_email`='" . mysqli_real_escape_string($con, $_SESSION['UserEmail']) . "' ";
	$result = $con->query($sql);
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_order` WHERE `payment`=1 AND `email`='" . mysqli_real_escape_string($con, $_SESSION['UserEmail']) . "' ORDER BY `order_id` DESC";
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_order` WHERE `order_id`='" . mysqli_real_escape_string($con, $id) . "' ";
	$result = $con->query($sql);
	$row = $result->fetch_assoc();
	return $row;
}
...
function viewproductitembycart($cart_id, $status) {
    global $con;
    // Query to get all items in the cart for a specific User_ID
    $sql = "SELECT * FROM `mobee_cart` WHERE `User_ID`='" . mysqli_real_escape_string($con, $cart_id) . "'";
    $result = $con->query($sql);
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $sql1 = "SELECT * FROM `mobee_product` WHERE `product_id`='" . $product_id . "'";
            $result1 = $con->query($sql1);

            if ($result1 && $result1->num_rows > 0) {
                while ($row1 = $result1->fetch_assoc()) {
                    // Display the product details based on the status
                    echo "<tr>
                            <td>" . htmlspecialchars($row1['product_name']) . "</td>
                            <td><img src='admin/Upload/Products/" . htmlspecialchars($row1['image']) . "' height='75px' width='75px'></td>
                            <td>" . htmlspecialchars($row1['selling']) . ".00/-</td>
                            <td>" . htmlspecialchars($row['qty']) . "</td>
                            <td>";
                    if ($status == "Delivered") {
                        echo "<a class='btn btn-danger' href='add_review.php?productid=" . base64_encode($row1['product_id']) . "'><span class='glyphicon glyphicon-star' aria-hidden='true'></span> Add Review</a>";
                    } else {
                        echo "<a disabled class='btn btn-danger' href='javascript:void(0);'><span class='glyphicon glyphicon-star' aria-hidden='true'></span> Add Review</a>";
                    }
                    echo "</td></tr>";
                }
            } else {
        ...
        echo "<tr><td colspan='5'>No items found in the cart.</td></tr>";
    }
}
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_productimage` WHERE `product_id`='" . mysqli_real_escape_string($con, $id) . "' ";
	$result = $con->query($sql);
	if ($result->num_rows > 0) {
		while ($row = $result->fetch_assoc()) {
			echo "
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_product` ORDER BY RAND()";
	$result = $con->query($sql);
...
{
	global $con;
	$sql = "SELECT * FROM `mobee_product` WHERE `product_name` LIKE '%" . $keyword . "%' LIMIT 20";
	$result = $con->query($sql);
