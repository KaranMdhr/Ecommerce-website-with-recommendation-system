...
{
	global $con;
	$sql="SELECT `product_id`, `product_name`, `image`, `price`, `selling`, `special`,`SKU` FROM `mobee_product` ";
	$result = $con->query($sql);
...
//This Function For View Product Details By ProductID
...
function ViewProductByID($id)
{
	...
	//sql
	$sql="SELECT * FROM `mobee_product` WHERE `product_id`='".mysqli_real_escape_string($con,$id)."' ";
	$result = $con->query($sql);
...
function ViewAccountHistory()
{
	...
	if ($result->num_rows > 0) {
		 while($row = $result->fetch_assoc()) {
		 	if($_SESSION['mobeeAdmin'] == $row['email'])
		 	{
			echo " <tr>
...
function viewpendingorder()
{
	...
	//sql
	$sql="SELECT `order_id`,`cart_id`,`order_by`,`totalamount`,`deliverycharge`,`mode`,`date`,`time` FROM `mobee_order` WHERE `payment`='1' AND `current_status`='Pending' ORDER BY `order_id` DESC ";
	$result = $con->query($sql);
...
function viewproductitembycart($cart_id)
{
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$sql1="SELECT product_id,product_name,image,selling,SKU FROM `mobee_product` WHERE `product_id`='".mysqli_real_escape_string($con,$row['product_id'])."'";
			$result1= $con->query($sql1);
			while($row1 = $result1->fetch_assoc())
			{
				echo "<tr>
...
{
	global $con;
	$sql="SELECT * FROM `mobee_order` WHERE `order_id`='".mysqli_real_escape_string($con,$order_id)."' ";
	$result=$con->query($sql);
	if($result->num_rows>0)
	{
		while($row = $result->fetch_assoc()) {
...
{
    global $con;
    $sql = "SELECT `order_id`, `cart_id`, `order_by`, `totalamount`, `deliverycharge`, `mode`, `date`, `time`, `current_status` FROM `mobee_order` WHERE `payment`='1' ORDER BY `order_id` DESC";
    $result = $con->query($sql);
                ...
                <td>
                    <form method='POST' action='update_order_status.php'>
                        ...
                        <select name='new_status' onchange='this.form.submit()'>
                            <option value='Order Confirmed'".($row['current_status'] == 'Order Confirmed' ? ' selected' : '').">Order Confirmed</option>
                            <option value='Shipped'".($row['current_status'] == 'Shipped' ? ' selected' : '').">Shipped</option>
                            <option value='Item On The Way'".($row['current_status'] == 'Item On The Way' ? ' selected' : '').">Item On The Way</option>
...
function viewpendingreview()
{
	...
	if ($result->num_rows > 0) {
		while($row=$result->fetch_assoc())
		{
			$sql1="SELECT `product_name`,`image` FROM `mobee_product` WHERE `product_id`='".$row['product_id']."' ";
			$result1=$con->query($sql1);
